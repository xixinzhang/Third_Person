...
from sandbox.bradly.third_person.policy.expert_reacher import load_expert
    ...
    if train:
        ...
        for iter_step in range(0, iterations):
            ...
            if ckpt_rew_mean> best_rew:
                ...
                with open(f'stu_reacher_{seed}_v{exp_idx}.pickle', 'wb') as handle:
            ...
            with open(f'train_reacher_{seed}_v{exp_idx}.npy', 'wb') as f:
                np.save(f, res_np)
            # plot figure
            plt.figure()
            sns.set_theme()
            ...
            plot.set_title('Reacher Third Person')
            fig=plot.get_figure()
            fig.savefig(f'res_reacher_{seed}_v{exp_idx}.png')
    else:
        # test
        res_policy = load_expert(f=f'stu_reacher_{seed}_v{exp_idx}.pickle')
