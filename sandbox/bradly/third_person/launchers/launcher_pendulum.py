...
from sandbox.bradly.third_person.algos.pendulum_trainer import CyberPunkTrainer
...
expert_env = TfEnv(normalize(GymEnv(PendulumEnv,{'color':'red'})))
novice_env = TfEnv(normalize(GymEnv(PendulumEnv,{'color':'blue'}), normalize_obs=True))
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    expert_policy = load_expert_inverted_dp('expert_pendulum.pickle')
    ...
    for iter_step in range(0, iterations):
        trainer.take_iteration(n_trajs_cost=1, n_trajs_policy=1)
